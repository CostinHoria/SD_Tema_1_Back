...
public class PatientController {
    ...
    public ResponseEntity<PatientDetailsDTO> deletePatient(@PathVariable("name") String patientName) {
        System.out.println("ID caregiver DELETED: "+patientName);
        PatientDetailsDTO dto = patientService.deletePatientByName(patientName);
        return new ResponseEntity<>(dto,HttpStatus.OK);
    }

    @PutMapping("/update")
    public ResponseEntity<UUID> updatePatient(@RequestBody PatientDetailsDTO patientDetailsDTO){
        UUID resultID = patientService.updatePatient(patientDetailsDTO);
        return new ResponseEntity<>(resultID,HttpStatus.CREATED);
    }

    @GetMapping("/caregiver/{name}")
    public ResponseEntity<List<Patient>> getPatientsByCaregiver(@PathVariable("name")String caregiverName){
        List<Patient> dtos = patientService.findByCaregiver(caregiverName);
